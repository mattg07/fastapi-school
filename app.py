...
VALID_PROGRAMS = []  # Will be populated by load_valid_programs or find_program
...
def recommend_schools(
    ...
    try:
        ...
        possible_name_cols = [c for c in merged.columns if c.startswith("name_")]
        if verbose and possible_name_cols:
            print(f"Possible name columns after first merge: {possible_name_cols}")
        ...
        # Rename if "name_col" reappears, or if "name_prog," "name_x," etc.
        if "name_col" in merged.columns:
            merged.rename(columns={"name_col": "name"}, inplace=True)
        ...
        # Filter out only those that have a tier, fallback if empty
        final_candidates = merged[merged["tier"].notnull()].copy()
        if final_candidates.empty:
            # If no schools are within the immediate GPA/SAT filter,
            # we skip to fallback logic below. But let's do a smaller fallback:
        ...
        def calculate_academic_distance(row, user_gpa, user_sat):
            ...
            distance ~ (|gpaDiff|/4 + |satDiff|/1600)/2
            """
            gpa_distance = 0.0
            sat_distance = 0.0
            count = 0
        ...
        def calculate_earnings_score(row):
            ...
            # weight 5-year more heavily if present
        ...
        # Additional composite-score adjustment
        ...
        final_candidates = final_candidates.sort_values(
            by=["composite_score", "data_completeness"],
            ascending=[False, False]
        )
        final_candidates["tier"] = final_candidates.apply(assign_tier, axis=1)

        ###########################################
        # FALLBACK LOGIC IF FEWER THAN 10 MATCHES
        ###########################################
        min_required = 10
        ...
        if count_final < min_required:
            ...
            # Compute missing numeric columns
            ...
            # Ensure fortune_500_companies is a list
...
async def get_recommendations(request: RecommendationRequest):
    try:
        ...
        # Convert DataFrame to list of dicts and clean for JSON
